...
- **The index rebuilds are pinned to a time** by drop-ins in
  [`etc/systemd/system/man-db.timer.d/`](./etc/systemd/system/man-db.timer.d/)
  and
  [`etc/systemd/system/plocate-updatedb.timer.d/`](./etc/systemd/system/plocate-updatedb.timer.d/),
  rather than midnight with hours of random delay. `OnCalendar` is a list, so
  each drop-in clears it before setting its own; `Persistent=false` drops the
  catch-up run after a boot that missed the slot, which for `updatedb` means
